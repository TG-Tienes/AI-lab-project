...
def SolveBFS(blockedList):
    ...
    while len(frontier) != 0:
        ...
        if blockedList[pos[0]][pos[1]] == ".":
            ...
            #fillPixel(-400 + (pos[1]) * 30, -250 + (R - 1 - pos[0]) * 30, 'Purple')
        for direct in direction: #1 diem co the di 4 vtri
            expanded = ""
            nr, nc = pos[0]+direct[0], pos[1]+direct[1] #vi tri diem tiep theo thu tu trai phai xuong
            if nr < 0 or nr >= R or nc < 0 or nc >= C or visited[nr][nc] == True or blockedList[nr][nc] == '%':
                continue
...
def Solve_Maze_Astar(blockedList):
    R, C = len(blockedList), len(blockedList[0])
    for i in range(R):
        for j in range(C):
            if blockedList[i][j] == "S": #start
                start = (i, j)
                break
        else:
            continue
